  ...
  <div class="view">
    <Panel :title="contestId ? this.$i18n.t('m.Contest_Problem_List') : this.$i18n.t('m.Problem_List')">
      ...
      <div class="panel-options">
        ...
        <el-button type="success" size="small" @click="bulkOperation('make_visible')" icon="el-icon-check">Bulk Visible
        </el-button>
        <el-button type="warning" size="small" @click="bulkOperation('make_invisible')" icon="el-icon-close">Bulk
          Invisible
        </el-button>
...
export default {
  ...
  methods: {
    ...
    bulkOperation(operation) {
      this.$confirm(`确认要执行批量操作"${operation === 'make_visible' ? '设为可见' : '设为不可见'}"吗?`, '确认操作', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        api.bulkOperation({ operation: operation }).then(response => {
